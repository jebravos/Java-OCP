...
public class DatesNTimes {
    ...
    private static void creatingDatesAndTimes() {
        ...
        PrintUtils.println("1.{}", ZonedDateTime.of(2019, 1, 30, 10, 10, 10, 0, ZoneId.of("US/Eastern")));
        // public static ZonedDateTime of(LocalDate date, LocalTime time, ZoneId zone)
        PrintUtils.println("2.{}", ZonedDateTime.of(LocalDate.now(), LocalTime.now(), ZoneId.of("US/Eastern")));
        // public static ZonedDateTime of(LocalDateTime dateTime, ZoneId zone)
        PrintUtils.println("3.{}", ZonedDateTime.of(LocalDateTime.now(), ZoneId.of("US/Eastern")));
        PrintUtils.println("-----------------------------");
        // Getting available zone Ids
        PrintUtils.println("Available zoneIDs: {}", ZoneId.getAvailableZoneIds());
    ...
    private static void manipulatingDAtesAndTimes() {
        ...
        PrintUtils.println("plus one day: {}", date);
        date = date.plusWeeks(1);
        PrintUtils.println("plus one week: {}", date);
        date = date.plusYears(1);
        PrintUtils.println("plus one year: {}", date);
        // same for go backward in time
        date = date.minusDays(10);
        PrintUtils.println("minus 10 days: {}", date);
        date = date.minusYears(10);
        PrintUtils.println("minus 10 years: {}", date);
        // Its common to chain methods
        date = date.plusYears(1).plusMonths(2).plusDays(15);
        PrintUtils.println("Chaining methods, plus 1 year 2 months and 15 days: {}", date);
        PrintUtils.println("-----------------------------");
    }
    ...
    private static void convertingToLong() {
        ...
        PrintUtils.println("LocalDate.now() epoch day, days since January 1, 1970: {}", date.toEpochDay());

        LocalDateTime dateTime = LocalDateTime.of(LocalDate.now(), LocalTime.now());
        ZonedDateTime zonedDateTime = ZonedDateTime.of(LocalDateTime.now(), ZoneId.systemDefault());
        //LocalDateTime and ZonedDateTime have toEpochSecond(), which is the number of
        //seconds since January 1, 1970.
        PrintUtils.println("LocalDateTime.of(LocalDate.now(), LocalTime.now()) epoch day, seconds since January 1, 1970: {}", zonedDateTime.toEpochSecond());
        PrintUtils.println("ZonedDateTime.of(LocalDateTime.now(), ZoneId.systemDefault()) epoch day, seconds since January 1, 1970: {}", dateTime.toEpochSecond(ZoneOffset.UTC));
        PrintUtils.println("-----------------------------");
    ...
    private static void workingWithPeriods() {
        ...
        LocalDate date = LocalDate.now();
        PrintUtils.println("actual date:{}", date);
        date = date.plus(periodOfYears);
        PrintUtils.println("plus a period of one year: {}", date);
        date = date.plus(periodOfDays);
        PrintUtils.println("plus a period of one day: {}", date);
    ...
    private static void workingWithDuration() {
        ...
        PrintUtils.println("Duration format ofHours(): {}", duration);
        PrintUtils.println("Duration format: ofSeconds(){}", everySeconds);
        // The number of seconds includes fractional seconds
        PrintUtils.println("Duration format: ofNanos(){}", everyNanos);
        ...
        PrintUtils.println("now time: {}", now);
        PrintUtils.println("now plus one hour: {}", now.plus(hourly));

        LocalDateTime nowDateTime = LocalDateTime.now();
        PrintUtils.println("now date time: {}", nowDateTime);
        PrintUtils.println("now date time: plus one hour {}", nowDateTime.plus(hourly));
        ...
        PrintUtils.println("difference in Hours between {} and {} : {}", now, now.plus(hourly), ChronoUnit.HOURS.between(now, now.plus(hourly)));
        PrintUtils.println("difference in minutes between {} and {} : {}", now, now.plus(hourly), ChronoUnit.MINUTES.between(now, now.plus(hourly)));
        PrintUtils.println("difference in seconds between {} and {} : {}", now, now.plus(hourly), ChronoUnit.SECONDS.between(now, now.plus(hourly)));
        PrintUtils.println("-----------------------------");
    }
    ...
    private static void workingWithInstants() {
        ...
        PrintUtils.println("Instant now is {}", now);
        // ... Something time consuming
        PrintUtils.sleep(Duration.ofSeconds(2).toMillis());
        //
        Instant later = Instant.now();
        PrintUtils.println("Time difference: {} millis", ChronoUnit.MILLIS.between(now, later));
        ...
        PrintUtils.println("Instant from epoch seconds: {}", fromEpochSeconds);
        ...
        PrintUtils.println("next day {}", nextDay);
        Instant nextMinute = now.plus(1, ChronoUnit.MINUTES);
        PrintUtils.println("next day {}", nextMinute);
        ...
        try {
            ...
            PrintUtils.println("now.plus(1, ChronoUnit.MONTHS) will throw {} - {}", e.getClass(), e.getMessage());
        }
